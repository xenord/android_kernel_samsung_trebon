	...
	if (!enabled) return;
        if (!suspend) { // resume at max speed:
			...
			for_each_cpu(cpu, &tmp_mask) {
				pcpu = &per_cpu(cpuinfo, cpu);
				smp_rmb();
				if (!pcpu->governor_enabled) continue;
					__cpufreq_driver_target(pcpu->policy, hispeed_freq, CPUFREQ_RELATION_L);
		}
		mutex_unlock(&set_speed_lock);
        pr_info("[imoseyon] interactivex2 awake at %d\n", pcpu->policy->cur);
        } else {
			mutex_lock(&set_speed_lock);
			for_each_cpu(cpu, &tmp_mask) {
				pcpu = &per_cpu(cpuinfo, cpu);
            ...
            mutex_unlock(&set_speed_lock);
            pr_info("[imoseyon] interactivex2 suspended at %d\n", pcpu->policy->cur);
        }
}
...
static int __init cpufreq_interactivex_init(void)
{
	...
	down_wq = alloc_workqueue("kinteractivex_down", 0, 1);
